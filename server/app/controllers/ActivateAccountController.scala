...
class ActivateAccountController @Inject() (
  ...
  def send(email: String) = UnsecuredAction.async { implicit request: Request[AnyContent] =>
    ...
    userService.retrieve(loginInfo).flatMap {
      ...
      case Some(user) => Future.successful(Redirect(Calls.signin).flashing("info" -> Messages("already.activation.confirmed")))
      case None => Future.successful(Redirect(Calls.signin).flashing("error" -> Messages("not.exists.credentials")))
    }
  }
