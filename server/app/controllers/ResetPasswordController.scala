...
class ResetPasswordController @Inject() (
  ...
  resetPassword: views.html.resetPassword)(implicit ex: ExecutionContext) extends SilhouetteController(scc) {
   ...
   */
  def view(token: UUID) = UnsecuredAction.async {
   ...
   * @return The result to display.
   */
  def submit(token: UUID) = UnsecuredAction.async {
    implicit request: Request[AnyContent] =>
