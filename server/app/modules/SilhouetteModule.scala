...
class SilhouetteModule extends AbstractModule with ScalaModule {
   ...
   */
  //@Provides
  //def provideGoogleTotpInfoDAO(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[GoogleTotpInfo] =
  //  new GoogleTotpInfoDAO(dbConfigProvider)

  @Provides
  def providePasswordInfoDAO(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[PasswordInfo] =
    new PasswordInfoDAO(dbConfigProvider)

  @Provides
  def provideOAuth1InfoDAO(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[OAuth1Info] =
    new OAuth1InfoDAO(dbConfigProvider)

  @Provides
  def provideOAuth2InfoDAO(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[OAuth2Info] =
    new OAuth2InfoDAO(dbConfigProvider)

  @Provides
  def provideOpenIDInfoDAO(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[OpenIDInfo] =
    new OpenIDInfoDAO(dbConfigProvider)
